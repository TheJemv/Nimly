-- Fix: en Supabase pgcrypto vive en el schema `extensions`, no en `public`.
-- Recreamos las funciones fijando search_path y calificando todo por completo
-- (recomendación de seguridad para SECURITY DEFINER).
...
begin
    ...
    values (auth.uid(), extensions.crypt(p_passcode, extensions.gen_salt('bf', 12)), 0, null, now())
...
begin
    ...
    v_ok := (v_row.passcode_hash = extensions.crypt(p_passcode, v_row.passcode_hash));
